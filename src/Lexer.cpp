...
char Lexer::peek(int offset) const {
    ...
    if (index >= source_.size()) return '\0';
...
bool Lexer::tryMatchMultiwordKeyword(const std::string& firstWord,
    ...
    if (combined == "SCRIPT AREA")   { outType = TokenType::SCRIPT_AREA;   return true; }
    if (combined == "START SCRIPT")  { outType = TokenType::START_SCRIPT;  return true; }
    if (combined == "END SCRIPT")    { outType = TokenType::END_SCRIPT;    return true; }
    if (combined == "START IF")      { outType = TokenType::START_IF;      return true; }
    if (combined == "END IF")        { outType = TokenType::END_IF;        return true; }
    if (combined == "START FOR")     { outType = TokenType::START_FOR;     return true; }
    if (combined == "END FOR")       { outType = TokenType::END_FOR;       return true; }
    if (combined == "START REPEAT")  { outType = TokenType::START_REPEAT;  return true; }
    if (combined == "END REPEAT")    { outType = TokenType::END_REPEAT;    return true; }
    if (combined == "REPEAT WHEN")   { outType = TokenType::REPEAT_WHEN;   return true; }
    if (combined == "ELSE IF")       { outType = TokenType::ELSE_IF;       return true; }
...
Token Lexer::scanChar() {
    ...
    char ch = advance();
    if (isAtEnd() || peek() != '\'') {
        throw std::runtime_error("CHAR literal must be exactly one character, line "
                                 + std::to_string(startLine));
    }
    advance(); // closing '
...
std::vector<Token> Lexer::tokenize() {
    ...
    while (!isAtEnd()) {
        ...
        switch (c) {
            case '+': tokens.emplace_back(TokenType::PLUS,        "+", line_); break;
            case '-': tokens.emplace_back(TokenType::MINUS,       "-", line_); break;
            case '*': tokens.emplace_back(TokenType::MULTIPLY,    "*", line_); break;
            case '/': tokens.emplace_back(TokenType::DIVIDE,      "/", line_); break;

            // Single % (not %%) — modulo operator
            case '%': tokens.emplace_back(TokenType::MODULO,      "%", line_); break;

            case '&': tokens.emplace_back(TokenType::AMPERSAND,   "&", line_); break;
            case '$': tokens.emplace_back(TokenType::DOLLAR,      "$", line_); break;
            case ':': tokens.emplace_back(TokenType::COLON,       ":", line_); break;
